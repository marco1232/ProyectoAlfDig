...
//$val= str_replace("'", "", $val);
//if(empty($val)){echo 'is empty<br>';}else{echo $val.'<br>';}
switch ($op){
    case 1:{
...
//                        $sql = "select * from usuario where usua='".$usua."' and pass='".$pass."' and estado='ACTIVO'";
                        $sql = "select * from usuario u inner join colaborador c on u.dniusu=c.dniusu order by id_cola";
                        $result = mysqli_query($conn, $sql);
//                        echo 'yes: '.$sql;
                        echo '<table border="1" class="tituloTabla" style="text-align: center" id="customers1">';
                                echo '<tr>';
                                    echo '<th colspan="30" class="tituloTabla">Colaboradores</th>';
                                echo '</tr>';
                        ...
                        while ($row = mysqli_fetch_assoc($result)) {
                            if ($row['estado'] == 'INACTIVO') {
                                        echo '<tr class="fondogrilla" style="background-color: #ff000061;"><td>'.$row['id_cola']."</td>";
                                    } else if ($row['estado'] == 'PENDIENTE') {
                                        echo '<tr class="fondogrilla" style="background-color: #ffff0059;"><td>'.$row['id_cola']."</td>";
                                    } else if ($row['estado'] == 'ACTIVO') {
                                        echo '<tr class="fondogrilla" ><td>'.$row['id_cola']."</td>";
                                    }
                                    ...
                                    echo "<td>".$row['cip']."</td>";
                                    echo "<td>".$row['gradoacad']."</td>";
                                    echo "<td>".$row['especia']."</td>";
                                    echo "<td>".$row['nomuniver']."</td>";
                                    echo "<td>".$row['descripdocen']."</td>";
                                    echo "<td>".$row['nivelconoci']."</td>";
                                    echo "<td>".$row['aplicarpara']."</td>";
                                    echo "<td>".$row['privilegiodesubirmat']."</td>";
                        }
...
//                        $fila = array();
//                        $sql = "select * from usuario where usua='".$usua."' and pass='".$pass."' and estado='ACTIVO'";
                        $sql = "select * from usuario u inner join colaborador c on ((u.nombres like '%".$val."%' or u.apellidos like '%".$val."%') ) and u.dniusu=c.dniusu order by id_cola";
                        $result = mysqli_query($conn, $sql);
//                        echo 'no: '.$sql;
                        echo '<table border="1" class="tituloTabla" style="text-align: center" id="customers1">';
                                echo '<tr>';
